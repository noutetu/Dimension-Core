...
public class SystemMessage : MonoBehaviour
{
    ...
    public static SystemMessage Instance; // シングルトンインスタンス
    [Header("矢印")]
    [SerializeField] public GameObject upArrowObject; // 上矢印オブジェクト
    [SerializeField] GameObject arrowPoint; // 矢印の位置
    ...
    // チュートリアルメッセージ表示
    // ==========================
    public static void ShowTutrialMessage()
    {
        ...
        Observable.Timer(System.TimeSpan.FromSeconds(4.5f)).Subscribe(_ =>
        {
            ...
            gameFlowManager.SelectPhase(SelectionMode.OnlyCommon, 3,true); // フェーズを選択
        }).AddTo(Instance);
    }
    ...
    public static void ShowUpArrow()
    {
        ...
        Instance.upArrowObject.transform.position = Instance.arrowPoint.transform.position;
        Instance.upArrowObject.SetActive(true); // 上矢印を表示
        // ログを出す
        DebugUtility.Log("ShowUpArrow");
    }
